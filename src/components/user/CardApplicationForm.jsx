...
import { PersonBadge, Buildings, CashCoin, Check2Circle, ChevronRight, ChevronLeft, HandThumbsUp, ShieldLock } from 'react-bootstrap-icons';

const CardApplicationForm = ({ user, onSubmit, onCancel, riskLevel = 'Medium' }) => {
                    ...
                    <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="flex flex-col gap-2">
                                ...
                                <input className="bg-slate-950/50 border border-white/10 rounded-xl px-4 py-3 text-slate-300 font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all cursor-not-allowed" type="text" value={formData.fullName} readOnly />
                            </div>
                            <div className="flex flex-col gap-2">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                                    <Buildings className="text-indigo-400" /> Employment Type
                                </label>
                                <select className="bg-slate-950/50 border border-white/10 rounded-xl px-4 py-3 text-white font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all" value={formData.employment} onChange={e => setFormData({ ...formData, employment: e.target.value })}>
                ...
                {step === 2 && (
                    <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                        <header className="mb-8">
                            ...
                            <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1">Select your preferred credit tier based on risk analysis</p>
                        </header>
                            ...
                            {eligibleCards.map(card => {
                                ...
                                return (
                                    <div key={card.Card_Name} onClick={() => setFormData({ ...formData, cardType: card.Card_Name })} className={`group relative p-6 rounded-2xl cursor-pointer border-2 transition-all duration-300 ${isSelected ? 'bg-indigo-500/10 border-indigo-500 shadow-[0_0_30px_rgba(99,102,241,0.15)]' : 'bg-white/5 border-white/5 hover:border-white/20'}`}>
                                        ...
                                        <h3 className="text-sm font-black text-white uppercase tracking-widest mb-1">{card.Card_Name}</h3>
                                        <p className="text-[10px] text-indigo-300 font-bold mb-3">{card.Card_Type}</p>
                ...
                {step === 3 && (
                    <div className="animate-in fade-in slide-in-from-right-4 duration-300 text-center py-6">
                        ...
                        <p className="text-slate-400 text-sm max-w-md mx-auto mb-10 leading-relaxed font-medium">
                            By clicking confirm, you authorize the credit department to run a secure assessment. Results typically materialize within <span className="text-white font-bold italic underline">24 standard hours</span>.
                        </p>
